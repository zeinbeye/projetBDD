...
{
   	$S_matricule=$_POST['matricule'];
    $S_marque=$_POST['marque'];
	$S_chauffeur=$_POST['chauffeur'];
       /*	$S_nom=1;
	$S_prenom=2;
	$S_tel=3;
	$S_nomVDep=4;
...
//$query ="INSERT INTO bus ( matricule , marque , nom_chauff  ) VALUES('$S_matricule', '$S_marque','$S_chauffeur')";
$query =" begin AJOUT_BUS('$S_matricule', '$S_marque','$S_chauffeur');end;";
?>
...
<form action="#" method="post">
  <div>
    ...
    <input type="text" id="name" name="matricule">
  </div>
   <div class="esp"></div>
   <div>
    <label for="name">Marque :</label>
    <input type="text" id="prenm" name="marque">
  </div>
   <div class="esp"></div>
   <?php
 $sql = 'SELECT nom_chauff FROM chauffeurs';
...
//while (($row = oci_fetch_array($stmt, OCI_ASSOC)) != false)
 ...
 ?>
  <div>
    <label for="mail">Chauffeur:</label>
    <select name="chauffeur"><?php while ($row = oci_fetch_assoc($stmt)){
 // print_r( $row);
echo '<option value="'.$row['NOM_CHAUFF'].'">'.$row['NOM_CHAUFF'].'</option>';
 }
?></select>
  </div>
   <div class="esp"></div>
